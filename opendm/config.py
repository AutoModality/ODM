...
def config(argv=None, parser=None):
    ...
    parser.add_argument('--band-alignment-local-homography',
            ...
            nargs=0,
            default=False,
            help=('Compute a local homography matrix for each individual raw image and use this matrix to warp the raw image. '
                  'Otherwise, select the best homography matrix and use this global matrix to warp all raw images. Default: '
                  '%(default)s'))

    parser.add_argument('--band-alignment-rig-optimization',
            action=StoreTrue,
            nargs=0,
            default=False,
            help=('Use enhanced correlation coefficient (ECC) algorithm to refine the warp matrix generated by the camera rig relatives '
                  'This option increases processing time. Default: '
                  '%(default)s'))
